...
M.util.init_block_hider = function(Y, config) {
    ...
    Y.use('base', 'node', 'anim', function(Y) {
            ...
            blockhider.prototype = {
                initializer : function(config) {
					...
					if (b.hasClass('hidden') &&
						!b.get('parentNode').get('parentNode').hasClass('has_dock') &&
						!(b.get('parentNode').get('parentNode').get('parentNode').get('parentNode').getAttribute('id') != 'region-main' &&
						b.hasClass('block_course_session'))) {
						c.setStyle('height','0');
    ...
    init : function(Y, nodeid) {
        ...
        if (node) {
            Y.use('node-menunav', function(Y) {
				...
				// [MDLUM-1373] - Optimiser le menu dynamique
				Y.Plugin.NodeMenuNav.ATTRS.submenuShowDelay = 0;
