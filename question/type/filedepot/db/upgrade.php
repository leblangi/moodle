...
/**
 * Filedepot question type upgrade code.
 *
 * @package    qtype
...
/**
 * Upgrade code for the filedepot question type.
 * @param int $oldversion the version we are upgrading from.
 */
function xmldb_qtype_filedepot_upgrade($oldversion) {
    global $CFG, $DB;

    $dbman = $DB->get_manager();

    if ($oldversion < 2011031000) {
        // Define table qtype_filedepot_options to be created
        $table = new xmldb_table('qtype_filedepot_options');

        // Adding fields to table qtype_filedepot_options
        ...
        // Adding keys to table qtype_filedepot_options
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('questionid', XMLDB_KEY_FOREIGN_UNIQUE,
                array('questionid'), 'question', array('id'));

        // Conditionally launch create table for qtype_filedepot_options
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // filedepot savepoint reached
        upgrade_plugin_savepoint(true, 2011031000, 'qtype', 'filedepot');
    }

    if ($oldversion < 2011060300) {
        // Insert a row into the qtype_filedepot_options table for each existing filedepot question.
        $DB->execute("
                INSERT INTO {qtype_filedepot_options} (questionid, responseformat,
                        responsefieldlines, attachments, graderinfo, graderinfoformat)
                SELECT q.id, 'editor', 15, 0, '', " . FORMAT_MOODLE . "
                FROM {question} q
                WHERE q.qtype = 'filedepot'
                AND NOT EXISTS (
                    SELECT 'x'
                    FROM {qtype_filedepot_options} qeo
                    WHERE qeo.questionid = q.id)");

        // filedepot savepoint reached
        upgrade_plugin_savepoint(true, 2011060300, 'qtype', 'filedepot');
    }
