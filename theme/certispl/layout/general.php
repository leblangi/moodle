 ...
 * @copyright  2013 Gilles-Philippe Leblanc - University of Montreal
...
$ishomeloggedoff = ($PAGE->bodyid == 'page-site-index' && !isloggedin());
$hassidepre = $PAGE->blocks->region_has_content('side-pre', $OUTPUT) && !$ishomeloggedoff;
$hassidepost = $PAGE->blocks->region_has_content('side-post', $OUTPUT) && !$ishomeloggedoff;
...
if ($hassideprefakeblock && !$hassidepostfakeblock) {
	$bodyclasses[] = 'side-pre-fakeblock';
} else if ($hassidepostfakeblock && !$hassideprefakeblock) {
	$bodyclasses[] = 'side-post-fakeblock';
} else if ($hassidepostfakeblock && $hassideprefakeblock) {
	$bodyclasses[] = 'both-side-fakeblock';
}	
